...
export function GameCard({
  ...
  const bodyBlock = (
    <Box sx={{ padding: "18px 20px 16px", display: "flex", flexDirection: "column", gap: 1, flex: 1 }}>
      ...
      <Typography component="div" sx={{ fontSize: 16, lineHeight: 1.5, color: "text.secondary", margin: "4px 0 0" }}>
        {children}
      </Typography>
    </Box>
  );
  ...
  const linkedContent = url ? (
    <Link href={url} target="_blank" rel="noopener noreferrer" sx={linkSx}>
      {imageBlock}
      {bodyBlock}
    </Link>
  ) : (
